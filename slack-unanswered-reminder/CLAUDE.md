...
- **チーム返信の判定は「スレッド返信(replies)内にチームメンバーの返信があるか」だけで行う**。このList の親投稿は全てボット `コア機能チームQA` が担当者を `<@…>` でメンションするため、**親自動投稿のメンションを「返信済み」と数えると、実際は未返信なのに取りこぼす実害がある**（例: ts `1782348248302719` は返信ゼロだが親投稿で担当 `U03C9BCE7` がメンションされており、旧ルールで誤除外された）。「被メンション=返信済み」扱いにするのは**スレッド返信内で人間がメンションして振った** User ID のみ（`team_members` 外でも可）。逆に、スレッド返信内にチームメンバーの返信があるのにそれを落とすと、既に対応済みの案件を誤って未返信報告する
