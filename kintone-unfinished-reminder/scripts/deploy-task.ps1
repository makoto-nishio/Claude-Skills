# Kintone チケットリマインダー - Task Scheduler デプロイスクリプト
...
foreach ($raw in $lines) {
    ...
    # 4 スペースインデント: サブキー（time / weekdays_only / hours / minutes / between など）
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description 'Kintoneから未対応チケットを取得し、Slackにリマインダーを投稿する。設定は config.yaml の schedule セクション（daily または interval）を参照。' | Out-Null
