# Slack 停滞スレッドリマインド - 実行スクリプト
# Task Scheduler から呼び出される。Claude Code CLI 経由で skill を実行する。
...
try {
    ...
    $prompt = @'
slack-unfinished-reminder skill の主処理を最後まで実行してください。

実行ステップ（順守）:
1. 作業ディレクトリの config.yaml を読み込む（list_name / notification_channel / team_members / min_elapsed_business_hours。min_elapsed_business_hours は未設定・0 以下なら 48）
2. SKILL.md の「実行ワークフロー」に従い、list_name から List の File ID を解決し、List(CSV) を読む
3. 「完了」が未チェック（true 以外）の項目を抽出
4. 抽出した未完了項のうち、メッセージリンクの ts（投稿時刻）から現在までの経過時間を「営業時間（土日除外・祝日は除外しない）」で算出し、min_elapsed_business_hours 営業時間以上のものだけに絞り込む（週末分・猶予内の項目は除外）。各項目は経過営業日で 🔴(>=20) / 🟡(>=10) / ⚪(>=2) に色分けする
5. スレッドの返信確認は行わない（未返信フィルタは廃止済み。slack_read_thread は呼ばない）
6. 対象が 1 件以上なら、notification_channel に通知メッセージを送信（親メッセージ=⚠️見出し+全員メンション+件数+説明+凡例／スレッド=担当者別の詳細・色マーカー付き）。0 件なら送信せず exit
...
SKILL.md の表現に多少の曖昧さがあっても、主処理（投稿チェックと通知送信）を最後まで完遂してください。
レビューやコメントは一切不要。実行結果のみ簡潔に報告してください。
'@

    # 無人実行のため、本 skill が使う具体的なツールを許可リストで明示する。
    # 範囲は SKILL.md の手順内で必要なものに限定（最小権限）。
    $allowedTools = @(
        # 対話セッション側の名前空間
        'mcp__plugin_slack_slack__slack_search_public'
        'mcp__plugin_slack_slack__slack_read_file'
        'mcp__plugin_slack_slack__slack_read_thread'
        'mcp__plugin_slack_slack__slack_read_channel'
        'mcp__plugin_slack_slack__slack_send_message'
        'mcp__plugin_slack_slack__slack_read_user_profile'
        'mcp__plugin_slack_slack__slack_search_users'
        # ヘッドレス（claude --print）側の名前空間。実測でこちらが使われる。
    ...
    # 実例: Slack MCP の名前空間不一致で全呼び出しが「権限未付与」となり、通知ゼロのまま正常終了した。
