...
Slack の QA List で「完了が未チェックで、投稿（スレッド作成）から一定の "営業時間"（既定 48 営業時間＝2 営業日。`min_elapsed_business_hours` で可変）以上経過した項目」を毎日（または指定間隔で）自動チェックし、別チャンネルへ担当者別のリマインダーを送信する Claude Code skill です。

経過時間は **営業時間**（土日を除外。祝日は除外しない）で数えます。各項目は経過営業日に応じて 🔴（20営業日以上）／🟡（10営業日以上）／⚪（2営業日以上）に色分けして表示します。投稿直後・週末分はまだ猶予扱いとして通知しません。

> **旧仕様からの変更**: かつては「チームの誰も返信していない（未返信）」項目に絞っていましたが、**この未返信フィルタは廃止**しました。現在はスレッドの返信状況を確認せず、「完了未チェック ＋ 48 営業時間以上経過」だけで対象を決めます。

> ⚠️ **よく似た別スキルと混同しないこと。** 本リポジトリは `slack-unfinished-reminder`（**停滞スレッドリマインド**）です。別ディレクトリに `slack-unanswered-reminder`（**未返信投稿リマインダー**）という非常によく似たスキルがあります。
>
> | | **本スキル** `slack-unfinished-reminder` | 別スキル `slack-unanswered-reminder` |
> |---|---|---|
> | 抽出条件 | 完了未チェック ＋ **投稿から 48 営業時間（可変）以上経過** | 完了未チェック ＋ **チームが未返信** |
> | 経過時間しきい値 | あり（`min_elapsed_business_hours`） | なし |
> | スレッド返信確認 | しない | する |
> | 色分け 🔴🟡⚪ | あり | なし |
> | タスク名 | `Claude-Slack-Unfinished-Reminder` | `Claude-Slack-Unanswered-Reminder` |
>
> 2 つは別ディレクトリ・別タスク名なので**同時に運用できます**。本 README・`config.yaml`・`scripts/` を編集するときは `slack-unfinished-reminder`（本スキル）側だけを触ってください。
...
- Slack の QA / 問い合わせ List を運用していて、「停滞案件（一定営業時間が過ぎても未完了のもの）」を定期的に拾いたい
...
3. 以降は指定スケジュールで全自動。対象（停滞）が 0 件なら通知は送らない
...
# 投稿からの経過 "営業時間"（土日除外）しきい値。これ以上経過した未完了項のみ通知（既定 48＝2営業日）
# 例: 24 にすると 24 営業時間（=1営業日）で通知対象になる。省略・0 以下なら 48 を使う
...
```powershell
cd C:\Users\<USER>\.claude\skills\slack-unfinished-reminder\scripts
.\deploy-task.ps1
```
...
[OK] Task registered: Claude-Slack-Unfinished-Reminder
     Schedule  : every 1 hour(s), starting 2026-05-28 16:06:10
...
| 項目 | 値 |
|------|-----|
| タスク名 | `Claude-Slack-Unfinished-Reminder` |
...
1. `logs\last_run.log` の最終行を確認
   - `done` で終わっていれば、対象（停滞）が 0 件で通知が省略されただけ
...
- **メンバー指定の柔軟性**: `team_members` は User ID と email を混在可。email は実行時に `slack_search_users` で User ID へ解決（完全一致 1 件のみ採用、0 件・複数件は無視）。親メッセージのメンションと、List「担当者」列（email）との突合に使う
- **経過時間は営業時間ベース**: 投稿（スレッド作成）からの経過を土日除外で算出（祝日は除外しない）。`min_elapsed_business_hours`（既定 48＝2 営業日）以上の未完了項のみ対象。スレッド返信の確認は行わない（旧「未返信」フィルタは廃止）
